...
_SYSTEM_PROMPT = (
    ...
    "iteration's layer-wise quantization/pruning config by calling the "
    f"`{_TOOL_NAME}` tool with one layer_configs entry per listed real "
    "stage name (set layer_name to the exact stage name given) -- respond "
    "with a tool call only, never plain text. You may confirm each stage's "
    "suggested candidate as-is, or adjust individual stages within the "
    "stated hw bounds if recent_failure_history shows a specific "
    "convergence problem the suggestion doesn't address (e.g. persistent "
